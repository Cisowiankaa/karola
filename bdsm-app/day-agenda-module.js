(()=>{
  ...
  function install(){ensureUI();['bdsm-offences-updated','bdsm-education-tasks-updated','bdsm-written-notes-updated','bdsm-hourly-reports-updated','bdsm-case-linked','bdsm-sync-complete'].forEach(ev=>document.addEventListener(ev,()=>{if(!document.querySelector('#view-day-agenda')?.classList.contains('hidden'))render()}));window.addEventListener('storage',()=>{if(!document.querySelector('#view-day-agenda')?.classList.contains('hidden'))render()});window.bdsmDayAgenda={open,refresh:render,date:()=>currentKey}}
