...
function ensureUI(){
  ...
  if(!document.querySelector('#dailyReportsStyles')){const st=document.createElement('style');st.id='dailyReportsStyles';st.textContent='.dr-head{display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap}.dr-toolbar{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.dr-toolbar input{background:#0a101a;color:#eef1f6;border:1px solid #283141;border-radius:8px;padding:9px}.dr-list{display:grid;gap:8px;margin-top:14px}.dr-row{display:grid;grid-template-columns:120px repeat(6,minmax(70px,1fr)) auto;gap:8px;align-items:center;padding:11px;border:1px solid #252d3c;border-radius:10px;background:#0c121c}.dr-detail{margin-top:14px}.dr-pills{display:flex;gap:8px;flex-wrap:wrap;margin:10px 0}.dr-pill{padding:6px 9px;border:1px solid #313b4f;border-radius:999px;font-size:11px}@media(max-width:1000px){.dr-row{grid-template-columns:110px 1fr auto}.dr-stat{display:none}}';document.head.appendChild(st)}
  if(!document.querySelector('#view-daily-reports')){const s=document.createElement('section');s.id='view-daily-reports';s.className='hidden';s.innerHTML='<div class="panel"><div class="dr-head"><div><h3 style="margin-bottom:4px">📅 Raport dzienny</h3><small style="color:#98a2b3">Osobny raport dla każdego dnia — tworzony automatycznie bez duplikatów.</small></div><div class="dr-toolbar"><input type="date" id="drDate"><button class="btn" id="drOpenDate">Otwórz datę</button><button class="btn" id="drToday">Dzisiaj</button><button class="btn primary" id="dailyReportRefresh">Odśwież dzisiaj</button></div></div><div id="dailyReportsList" class="dr-list"></div><div id="dailyReportDetail" class="dr-detail"></div></div>';content.appendChild(s);
    s.querySelector('#dailyReportRefresh').onclick=()=>{const d=keyOf(new Date());ensureDate(d);render();renderDetail(d)};
    s.querySelector('#drToday').onclick=()=>{const d=keyOf(new Date());s.querySelector('#drDate').value=d;renderDetail(d)};
    s.querySelector('#drOpenDate').onclick=()=>{const d=s.querySelector('#drDate').value;if(d){ensureDate(d);render();renderDetail(d)}};
...
function openDaily(){
  catchUp();ensureUI();document.querySelectorAll('.content > section').forEach(x=>x.classList.add('hidden'));document.querySelector('#view-daily-reports')?.classList.remove('hidden');document.querySelectorAll('#nav button').forEach(x=>x.classList.remove('active'));const b=[...document.querySelectorAll('#nav button')].find(x=>x.textContent.includes('Raport dzienny'));b?.classList.add('active');const d=keyOf(new Date()),inp=document.querySelector('#drDate');if(inp)inp.value=d;render();renderDetail(d);
}
function render(){
  ensureUI();const box=document.querySelector('#dailyReportsList');if(!box)return;const all=reports().slice(0,31);
...
function printReport(date){
  const x=reports().find(r=>r.date===date);if(!x)return;const q=window.open('','_blank');if(!q)return;const label=new Date(date+'T12:00:00').toLocaleDateString('pl-PL',{weekday:'long',day:'2-digit',month:'long',year:'numeric'});
...
function install(){
  catchUp();ensureUI();document.addEventListener('click',e=>{const b=e.target.closest?.('#nav button');if(!b||!b.textContent.includes('Raport dzienny'))return;e.preventDefault();e.stopImmediatePropagation();openDaily()},true);
  ['bdsm-day-agenda-updated','bdsm-offences-updated','bdsm-education-tasks-updated','bdsm-written-notes-updated','bdsm-hourly-reports-updated','bdsm-sync-complete'].forEach(ev=>document.addEventListener(ev,()=>{ensureDate(keyOf(new Date()));render()}));
  window.addEventListener('focus',()=>{catchUp();render()});document.addEventListener('visibilitychange',()=>{if(!document.hidden){catchUp();render()}});setInterval(()=>{catchUp();render()},3600000);window.bdsmDailyReports={list:reports,refresh:()=>{catchUp();render()},open:openDaily,openDate:renderDetail,print:printReport};
}
