(()=>{
  ...
  const trend=(a,b,inverse=false)=>{if(a===b)return{arrow:'→',cls:'same'};const up=a>b;const good=inverse?!up:up;return{arrow:up?'↑':'↓',cls:good?'good':'bad'}};
  function render(){
    ...
    if(!document.querySelector('#dashboardMonthCompareStyles')){const s=document.createElement('style');s.id='dashboardMonthCompareStyles';s.textContent='.dmc-grid{display:grid;grid-template-columns:repeat(5,minmax(120px,1fr));gap:10px}.dmc-card{padding:12px;border:1px solid #252d3c;border-radius:12px;background:#111722}.dmc-card b{display:block;font-size:20px;margin-top:4px}.dmc-sub{font-size:11px;color:#98a2b3}.dmc-trend{font-size:12px;margin-top:6px}.dmc-trend.good{color:#8bdc9b}.dmc-trend.bad{color:#ff9b9b}.dmc-trend.same{color:#cbd5e1}@media(max-width:900px){.dmc-grid{grid-template-columns:repeat(2,1fr)}}';document.head.appendChild(s)}
    ...
    box.innerHTML=`<div class="dtp-head"><h3>📅 Miesiąc vs poprzedni miesiąc</h3><span class="dtp-count">porównanie informacyjne</span></div><div class="dmc-grid">${items.map(([label,k,inv])=>{const t=trend(cur[k],prev[k],inv),suffix=k==='checklist'?'%':'';return `<div class="dmc-card"><span class="dmc-sub">${label}</span><b>${cur[k]}${suffix}</b><div class="dmc-trend ${t.cls}">${t.arrow} poprzednio: ${prev[k]}${suffix}</div></div>`}).join('')}</div><div class="dws-foot">Trend nie uruchamia żadnych automatycznych konsekwencji ani zmian terminów.</div>`;
  }
