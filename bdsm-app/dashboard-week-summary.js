...
(()=>{
  ...
  function one(offset){const events=read(K.events,[]),tasks=read(K.tasks,[]),agenda=read(K.agenda,{});const points=events.filter(x=>inside(x.start||x.created_at||x.updated_at,offset)).reduce((s,x)=>s+(Number(x.points_delta??x.points??0)||0),0);const tasksDone=tasks.filter(x=>done(x.status)&&inside(x.updated_at||x.completed_at||x.due_at||x.created_at,offset)).length;let checked=0,total=0;const start=weekStart(offset);for(let i=0;i<7;i++){const d=new Date(start);d.setDate(start.getDate()+i);const list=Array.isArray(agenda?.[dayKey(d)]?.checklist)?agenda[dayKey(d)].checklist:[];total+=list.length;checked+=list.filter(x=>x.done).length}return{offset,points,tasksDone,checklist:total?Math.round(checked/total*100):0,start}}
  const label=d=>d.toLocaleDateString('pl-PL',{day:'2-digit',month:'2-digit'});
  const maxAbs=a=>Math.max(1,...a.map(x=>Math.abs(x)));
  function render(){const dash=document.querySelector('#view-dashboard');if(!dash)return;let box=document.querySelector('#dashboardFourWeekTrend');if(!box){box=document.createElement('div');box.id='dashboardFourWeekTrend';box.className='panel';const ref=document.querySelector('#dashboardWeekSummary');if(ref&&ref.nextSibling)dash.insertBefore(box,ref.nextSibling);else dash.appendChild(box)}if(!document.querySelector('#dashboardFourWeekTrendStyles')){const s=document.createElement('style');s.id='dashboardFourWeekTrendStyles';s.textContent='.dfw-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-top:10px}.dfw-week{border:1px solid #252d3c;border-radius:12px;padding:10px;background:#111722}.dfw-week h4{margin:0 0 8px;font-size:12px}.dfw-metric{margin:8px 0}.dfw-metric-head{display:flex;justify-content:space-between;gap:8px;font-size:11px;color:#cbd5e1}.dfw-track{height:7px;background:#222a39;border-radius:999px;overflow:hidden;margin-top:5px}.dfw-fill{height:100%;background:currentColor}.dfw-points{color:#a5b4fc}.dfw-tasks{color:#86efac}.dfw-check{color:#facc15}.dfw-note{margin-top:10px;font-size:11px;color:#98a2b3}@media(max-width:900px){.dfw-grid{grid-template-columns:repeat(2,1fr)}}';document.head.appendChild(s)}const weeks=[-3,-2,-1,0].map(one),pm=maxAbs(weeks.map(x=>x.points)),tm=Math.max(1,...weeks.map(x=>x.tasksDone));box.innerHTML=`<div class="dtp-head"><h3>📈 Trend 4 tygodni</h3><span class="dtp-count">punkty • zadania • checklisty</span></div><div class="dfw-grid">${weeks.map((w,i)=>`<div class="dfw-week"><h4>${i===3?'Bieżący tydzień':'Tydzień'} · ${label(w.start)}</h4><div class="dfw-metric dfw-points"><div class="dfw-metric-head"><span>Punkty</span><b>${w.points>0?'+':''}${w.points}</b></div><div class="dfw-track"><div class="dfw-fill" style="width:${Math.round(Math.abs(w.points)/pm*100)}%"></div></div></div><div class="dfw-metric dfw-tasks"><div class="dfw-metric-head"><span>Wykonane zadania</span><b>${w.tasksDone}</b></div><div class="dfw-track"><div class="dfw-fill" style="width:${Math.round(w.tasksDone/tm*100)}%"></div></div></div><div class="dfw-metric dfw-check"><div class="dfw-metric-head"><span>Checklisty</span><b>${w.checklist}%</b></div><div class="dfw-track"><div class="dfw-fill" style="width:${w.checklist}%"></div></div></div></div>`).join('')}</div><div class="dfw-note">Wykres jest informacyjny. Nie powoduje automatycznych zmian punktów, terminów ani konsekwencji.</div>`}
