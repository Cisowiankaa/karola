(()=>{
  ...
  function stats(days,block=0){
    ...
    return {
      tasks:tasks.filter(x=>done(x.status)&&inside(x.updated_at||x.completed_at||x.due_at||x.created_at,days,block)).length,
      offences:offences.filter(x=>inside(x.occurred_at||x.created_at||x.updated_at,days,block)).length,
      notes:notes.filter(x=>inside(x.issued_at||x.date||x.created_at||x.updated_at,days,block)).length,
      points:events.filter(x=>inside(x.start||x.created_at||x.updated_at,days,block)).reduce((s,x)=>s+(Number(x.points_delta??x.points??0)||0),0),
