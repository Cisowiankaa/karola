(() => {
  ...
  const writeInviteLog = (log) => writeJSON(INVITE_LOG_KEY, log);
  ...
  function installEmailPanel() {
    ...
    section.innerHTML='<div class="panel"><h3>E-maile / Zaproszenia</h3><p style="color:#98a2b3;font-size:12px;margin-top:-4px">Historia zaproszeń z tej przeglądarki.</p><div id="emailInviteStats" style="margin:12px 0"></div><div id="emailInviteTable"></div></div>';
  ...
  function renderEmailPanel() {
    ...
    const sent=list.filter(i=>(log[i.person]||{}).status==='sent').length, errors=list.filter(i=>(log[i.person]||{}).status==='error').length;
  ...
  function install() {
    ...
    replacement.addEventListener('click',async()=>{
      ...
      const events=readJSON('bdsm-app-events-v3',[]),rules=readJSON('bdsm-app-rules-v3',{}),list=accessList();
      ...
      replacement.disabled=true;list.filter(x=>!x.revoked).forEach(x=>updateInviteStatus(x.person,{status:'sending',lastAttemptAt:new Date().toISOString(),requestId,error:null}));
      if(txt)txt.textContent='Synchronizacja… '+requestId;if(dot)dot.className='sync-dot';
      try{const result=await postSync(payload);writeJSON('bdsm-app-last-sync',{requestId,at:new Date().toISOString(),result:result.data||null});const sentAt=new Date().toISOString();list.filter(x=>!x.revoked).forEach(x=>updateInviteStatus(x.person,{status:'sent',sentAt,requestId,error:null}));if(txt)txt.textContent='Online — zapis potwierdzony • '+new Date().toLocaleTimeString('pl-PL');if(dot)dot.className='sync-dot ok';}
      catch(error){list.filter(x=>!x.revoked).forEach(x=>updateInviteStatus(x.person,{status:'error',lastAttemptAt:new Date().toISOString(),requestId,error:error.message||String(error)}));if(txt)txt.textContent='Błąd synchronizacji: '+(error.name==='AbortError'?'timeout':error.message);if(dot)dot.className='sync-dot';}
