...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const appId = String(process.env.META_FACEBOOK_APP_ID || '').trim();
    const appSecret = String(process.env.META_FACEBOOK_APP_SECRET || '').trim();
    if (!appId || !appSecret) return res.status(503).send('Brak META_FACEBOOK_APP_ID lub META_FACEBOOK_APP_SECRET w Vercel.');
