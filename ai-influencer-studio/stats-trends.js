(() => {
  ...
  function periodComparison(days){
    ...
    const currentItems=periodItems(day(currentStart),day(end)),previousItems=periodItems(day(prevStart),day(prevEnd));
    const current=summarize(currentItems),previous=summarize(previousItems);
    ...
    return {current,previous,currentItems,previousItems,changes,verdict,currentRange:`${day(currentStart)} – ${day(end)}`,previousRange:`${day(prevStart)} – ${day(prevEnd)}`};
  }
  function changeText(v){return `${v>0?'+':''}${v.toFixed(1)}%`;}
  function changeClass(v){return v>0?'stats-delta-up':v<0?'stats-delta-down':'';}
  function formatName(x){const t=String(x?.type||'Post').toLowerCase();return /reel|video/.test(t)?'Reels':/carousel/.test(t)?'Carousel':'Post';}
  function formatSummary(items){
    const groups={};items.forEach(x=>{const k=formatName(x),g=groups[k]||(groups[k]={type:k,count:0,score:0,comments:0,likes:0});g.count++;g.score+=itemScore(x);g.comments+=num(x.comments);g.likes+=num(x.likes)});
    return Object.values(groups).map(g=>({...g,avg:g.count?g.score/g.count:0})).sort((a,b)=>b.avg-a.avg);
  }
  function insights(days){
    const c=periodComparison(days),curF=formatSummary(c.currentItems),prevF=formatSummary(c.previousItems),best=curF[0]||null;
    const prevBy=Object.fromEntries(prevF.map(x=>[x.type,x]));
    const notes=[];
    if(best){const old=prevBy[best.type];const change=old&&old.avg?((best.avg-old.avg)/old.avg*100):null;notes.push({kind:'Format',title:`${best.type} prowadzi`,detail:change==null?`Najwyższy średni wynik w bieżącym okresie: ${best.avg.toFixed(1)}.`:`Średni wynik ${best.type} zmienił się o ${change>0?'+':''}${change.toFixed(1)}% względem poprzedniego okresu.`});}
    if(c.changes.count>10&&c.changes.score<-5)notes.push({kind:'Tempo',title:'Więcej publikacji, słabszy wynik',detail:'Częstotliwość wzrosła, ale łączny wynik treści spadł. W kolejnym tygodniu ogranicz liczbę testów i powtórz najlepszy format.'});
    else if(c.changes.count<-10&&c.changes.score>5)notes.push({kind:'Tempo',title:'Mniej treści, lepsza jakość',detail:'Publikacji było mniej, a wynik wzrósł. Warto utrzymać selektywniejszy rytm i skalować najlepsze motywy.'});
    else notes.push({kind:'Tempo',title:'Rytm publikacji stabilny',detail:`Zmiana liczby publikacji: ${changeText(c.changes.count)}. Kontynuuj rytm i testuj pojedyncze zmiany.`});
    const curComments=c.currentItems.reduce((s,x)=>s+num(x.comments),0),prevComments=c.previousItems.reduce((s,x)=>s+num(x.comments),0),commentChange=prevComments?((curComments-prevComments)/prevComments*100):(curComments?100:0);
    if(commentChange<-10)notes.push({kind:'Komentarze',title:'Komentarze spadły',detail:`Komentarzy jest o ${Math.abs(commentChange).toFixed(1)}% mniej. Dodaj jedno proste pytanie lub CTA do dyskusji.`});
    else if(commentChange>10)notes.push({kind:'Komentarze',title:'Komentarze rosną',detail:`Komentarzy jest o ${commentChange.toFixed(1)}% więcej. Powtarzaj pytania i formaty, które wywołują rozmowę.`});
    else notes.push({kind:'Komentarze',title:'Komentarze bez dużej zmiany',detail:'Zaangażowanie w dyskusję jest stabilne. Testuj różne CTA, ale zmieniaj tylko jeden element naraz.'});
    const recommendation=best?`Na następny tydzień: zaplanuj 3–4 publikacje, z czego co najmniej 2 w formacie ${best.type}. Zachowaj najlepszy temat, dodaj pytanie do komentarza i porównaj wynik po 7 dniach.`:'Na następny tydzień: zbierz minimum 3 publikacje z reakcjami. Aplikacja wtedy wskaże najlepszy format i kierunek skalowania.';
    return {notes:notes.slice(0,3),recommendation};
  }
  ...
  function insightsHtml(days){
    const x=insights(days);return `<div class="stats-insights"><div class="stats-insights-head"><div><b>Automatyczne wnioski</b><span>Reguły lokalne na podstawie zmian w wynikach</span></div><span class="stats-insights-badge">0 TOKENÓW</span></div><div class="stats-insights-grid">${x.notes.map(n=>`<div class="stats-insight"><em>${esc(n.kind)}</em><strong>${esc(n.title)}</strong><small>${esc(n.detail)}</small></div>`).join('')}</div><div class="stats-next-week"><b>Plan na kolejny tydzień:</b> ${esc(x.recommendation)}</div></div>`;
  }
  function renderRange(days){
    ...
    const target=q('#statsTrendDynamic',box);if(target)target.innerHTML=`<div class="stats-trend-grid"><div class="stats-trend-box"><span>Wzrost obserwujących</span><strong>${esc(dtext(d))}</strong></div><div class="stats-trend-box"><span>Publikacje w okresie</span><strong>${fmt(a.count)}</strong></div><div class="stats-trend-box"><span>Interakcje w okresie</span><strong>${fmt(a.interactions)}</strong></div></div>${chart(s,days)}${comparisonHtml(days)}${insightsHtml(days)}<div class="stats-trend-caption">Zakres ${days} dni. Dane liczone lokalnie z historii profilu i zapisanych publikacji — bez OpenAI i bez tokenów.</div>`;
  }
  function render(){
    ...
    wrap.innerHTML='<div class="stats-trends90-head"><div><b>Trendy 7 / 30 / 90 dni</b><span>Historia wzrostu, porównanie okresów i lokalne rekomendacje — 0 tokenów</span></div><div class="stats-range-tabs"><button data-stats-range="7">7 dni</button><button data-stats-range="30">30 dni</button><button class="active" data-stats-range="90">90 dni</button></div></div><div id="statsTrendDynamic"></div>';
  ...
  window.AIIStatsTrends={refresh:render,range:renderRange,compare:periodComparison,insights};
})();